...
interface BrokerSelectListProps {
...
const BrokerSelectList = ({
  ...
  variant,
  onChange
}: BrokerSelectListProps) => {
  const [init, setInit] = useState(false)
  const [loading, setLoading] = useState(false)
  const [brokers, setBrokers] = useState<movininTypes.Option[]>([])
  ...
  const getOptions = (users: movininTypes.User[]): movininTypes.Option[] =>
    users.map((user) => {
  ...
  const fetchData = async (_page: number, _keyword: string, onFetch?: movininTypes.DataEvent<movininTypes.User>) => {
    try {
      ...
      const payload: movininTypes.GetUsersBody = {
        user: '',
        types: [movininTypes.UserType.Broker],
      }
      ...
      const optionsList = getOptions(_data.resultData)
      const _brokers = _page === 1 ? optionsList : [...brokers, ...optionsList]
...
export default BrokerSelectList
