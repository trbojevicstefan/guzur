...
interface ListingStatusFilterProps {
  className?: string
  collapse?: boolean
  onChange?: (value: movininTypes.ListingStatus[]) => void
}

const statuses = helper.getListingStatuses()
...
const ListingStatusFilter = ({
  ...
  const [checkedStatuses, setCheckedStatuses] = useState<movininTypes.ListingStatus[]>([])
  const [allChecked, setAllChecked] = useState(false)
  const refs = useRef<(HTMLInputElement | null)[]>([])

  const handleChange = (_checkedStatuses: movininTypes.ListingStatus[]) => {
    if (onChange) {
  ...
  const handleCheckStatusChange = (e: React.ChangeEvent<HTMLInputElement> | React.MouseEvent<HTMLElement>) => {
    const status = e.currentTarget.getAttribute('data-value') as movininTypes.ListingStatus
        ...
        <ul className="status-list">
          {statuses.map((status, index) => (
            <li key={status.value}>
              ...
              />
              <ListingStatus value={status.value} />
            </li>
          ))}
        </ul>
