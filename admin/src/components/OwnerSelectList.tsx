...
interface OwnerSelectListProps {
...
const OwnerSelectList = ({
  ...
  variant,
  onChange
}: OwnerSelectListProps) => {
  const [init, setInit] = useState(false)
  const [loading, setLoading] = useState(false)
  const [owners, setOwners] = useState<movininTypes.Option[]>([])
  ...
  const fetchData = async (_page: number, _keyword: string, onFetch?: movininTypes.DataEvent<movininTypes.User>) => {
    try {
      ...
      const payload: movininTypes.GetUsersBody = {
        user: '',
        types: [movininTypes.UserType.Owner],
      }
      ...
      const _owners = _page === 1 ? optionsList : [...owners, ...optionsList]
...
export default OwnerSelectList
