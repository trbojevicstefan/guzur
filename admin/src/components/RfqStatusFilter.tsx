...
interface RfqStatusFilterProps {
  className?: string
  collapse?: boolean
  onChange?: (value: movininTypes.RfqStatus[]) => void
}

const statuses = helper.getRfqStatuses()
const allStatuses = statuses.map((status) => status.value)

const RfqStatusFilter = ({
  className,
  collapse,
  onChange
}: RfqStatusFilterProps) => {
  const [checkedStatuses, setCheckedStatuses] = useState<movininTypes.RfqStatus[]>([])
  const [allChecked, setAllChecked] = useState(false)
  const refs = useRef<(HTMLInputElement | null)[]>([])

  const handleChange = (_checkedStatuses: movininTypes.RfqStatus[]) => {
    if (onChange) {
  ...
  const handleCheckStatusChange = (e: React.ChangeEvent<HTMLInputElement> | React.MouseEvent<HTMLElement>) => {
    const status = e.currentTarget.getAttribute('data-value') as movininTypes.RfqStatus
...
export default RfqStatusFilter
