...
describe('Test database initialization', () => {
  it('should initialize database', async () => {
    ...
    await lv2.save()
    const l1 = new Location({ country: testHelper.GetRandromObjectIdAsString(), values: [lv1._id.toString(), lv2._id.toString()] })
    await l1.save()
    const l2 = new Location({ country: testHelper.GetRandromObjectIdAsString(), values: [lv2._id.toString()] })
    await l2.save()
    ...
    await cv2.save()
    const c1 = new Country({ values: [cv1._id.toString(), cv2._id.toString()] })
    await c1.save()
    const c2 = new Country({ values: [cv2._id.toString()] })
    await c2.save()
    ...
    const location1 = await Location.findById(l1._id.toString())
    const location2 = await Location.findById(l2._id.toString())
    ...
    const country1 = await Country.findById(c1._id.toString())
    const country2 = await Country.findById(c2._id.toString())
...
describe('initialize', () => {
  it('logs error when some routines fail', async () => {
    ...
    jest.unstable_mockModule('../src/utils/databaseLangHelper.js', () => ({
      ...
      initializeCountries: jest.fn(() => Promise.resolve(false)),
    }))
