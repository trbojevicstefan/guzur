...
const UpdateListing = () => {
  ...
  const [tempSecondaryImages, setTempSecondaryImages] = useState<string[]>([])
  ...
  useEffect(() => {
    const fetchProperty = async () => {
      ...
      try {
        ...
        setProperty(data)
        setName(data.name)
        setListingType(data.listingType || movininTypes.ListingType.Both)
        setPropertyType(data.type as movininTypes.PropertyType)
        setDescription(data.description)
        setAiDescription(data.aiDescription || '')
        setUseAiDescription(!!data.useAiDescription)
        setImage(data.image || '')
        setSecondaryImages(data.images || [])
        setLocation({ _id: data.location?._id, name: data.location?.name })
        ...
        setKitchens(String(data.kitchens ?? 1))
        setParkingSpaces(String(data.parkingSpaces ?? 0))
        ...
        setRentalTerm(data.rentalTerm || movininTypes.RentalTerm.Monthly)
        setSeoTitle(data.seoTitle || '')
        setSeoDescription(data.seoDescription || '')
        setSeoKeywords(data.seoKeywords || [])
        if (data.developmentId) {
          const devId = typeof data.developmentId === 'string' ? data.developmentId : (data.developmentId as movininTypes.Development)._id
  ...
  const handleUploadSecondary = async (files?: FileList | null) => {
    ...
    try {
      ...
      setTempSecondaryImages((prev) => [...prev, ...uploaded])
    } catch (err) {
      helper.error(err)
    } finally {
      setLoading(false)
  ...
  const removeSecondaryImage = async (filename: string) => {
    try {
      setLoading(true)
      if (tempSecondaryImages.includes(filename)) {
        await PropertyService.deleteTempImage(filename)
        setTempSecondaryImages((prev) => prev.filter((img) => img !== filename))
      }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    try {
      ...
      const payload: movininTypes.UpdatePropertyPayload = {
        _id: property._id,
        name,
        agency: property.agency?._id || user._id,
        ...
        rentalTerm,
        listingType,
        listingStatus: user.approved
          ? (property.listingStatus || movininTypes.ListingStatus.Published)
    ...
    <Layout strict onLoad={onLoad}>
      <div className="listing-form" ref={revealRef}>
        <form onSubmit={handleSubmit} className="listing-form-card">
          <h1 data-reveal>{listingStrings.UPDATE_HEADING || listingStrings.HEADING}</h1>
          {user && !user.approved && (
