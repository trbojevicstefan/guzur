...
- 2026-06-04: Established editorial-luxury design system + motion layer. Enriched `frontend/src/assets/css/theme.css` with full tokens (warm-neutral palette, accent ramp, fluid type scale, elevation tiers, motion easings, spacing scale), refined focus/selection and reusable utilities (`.u-kicker`, `.u-divider`, `.u-lift`, `.u-sheen`, `.btn-luxe`). Added reusable GSAP/ScrollTrigger hooks `frontend/src/hooks/useMotion.ts` (`useReveal`, `useParallax`, `useMagnetic`) — all reduced-motion-aware. Wired Home as the flagship: scroll reveals on sections/cards, staggered grid reveals, hero parallax, premium card hover lifts + button sheen (`frontend/src/pages/Home.tsx`, `frontend/src/assets/css/home.css`). Header/footer/sidebar untouched.
- 2026-06-04: Propagated the motion layer (`useReveal` + `data-reveal`/`data-reveal-group` markers) across all 46 customer frontend pages — content pages (About, Search, Property, Locations, Concierge), developments (Projects, Project, ProjectBrowse, Developer, DeveloperOrg, DeveloperOrganizations), partners (Brokerages, Brokerage, Agencies via re-export, Contact, Onboarding), account/booking (Bookings, Booking, Checkout, CheckoutSession, Dashboard, Settings, Notifications), legal/utility/auth (CookiePolicy, Privacy, ToS, Info, Error, NoMatch, Activate, ForgotPassword, ResetPassword, ChangePassword, Messages, Rfq, SignIn, SignUp, RoleSignUp), and dashboards (Broker/Developer/Owner, MyListings, Create/UpdateListing, CreateDevelopment, Organization). Motion-only/additive: no logic, text, or CSS changed; Header/Footer/sidebar untouched.
**Frontend Pages**
